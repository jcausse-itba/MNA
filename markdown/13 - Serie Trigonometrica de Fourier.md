...
Estas funciones periódicas deben ser funciones $f: \R \rightarrow \R$ periódicas de período $L$, es decir:
...
$$
\omega_n = \frac{2 \pi}{L} n = 2 \pi f_n \qquad \text{y} \qquad \omega_{n + 1} \ge \omega_n
$$

$$
\frac{a_0}{2} = \frac{1}{L} \int_L f(t) dt
$$

$$
a_n = \frac{2}{L} \int_L f(t) \cos(\omega_n t) dt
$$

$$
b_n = \frac{2}{L} \int_L f(t) \sin(\omega_n t) dt
$$

> **Nota:** Todas las integrales son sobre intervalos de longitud igual a un período de la función, puede ser $[0, L]$, $[-L/2, L/2]$, etc. El intervalo puede elegirse de la forma que resulte más conveniente.
...
### 13.1.2. Ejemplo de cómo hallar el período

Dada $f: \R \to \R / f(t) = \cos(t/3) + \cos(t/4)$, determinemos su período $L$.

$$
f(t) = f(t + L) \implies \cos \left( \frac{t}{3} \right) + \cos \left( \frac{t}{4} \right) = \cos \left( \frac{t+L}{3} \right) + \cos \left( \frac{t+L}{4} \right)
$$

Como $\cos(\theta) = \cos(\theta + 2k\pi), \forall k \in \Z$, entonces:

$$ 
\frac{L}{4} = 2m\pi \land \frac{L}{3} = 2n\pi \implies L = 8m\pi = 6n\pi \implies L = 4m = 3n 
$$

Los menores valores de $m$ y $n$ que satisfacen esta ecuación son $m = 3$ y $n = 4$. Por lo tanto, el mínimo valor de $L$ es:

$$
L = 24 \pi
$$

> **Nota:** La función $\cos(\omega_a t) + \cos(\omega_b t)$ solo es periódica si $\omega_a / \omega_b$ es un número racional.
...
$$
Donde:
* $\omega_n = \frac{2 \pi}{L} n = 2 \pi f_n$

> **Nota:** En Métodos Numéricos Avanzados, siempre elegiremos la forma trigonométrica, por simplicidad.
...
* $c_0 = a_0 / 2$
* $c_n = (a_n - i b_n) / 2$
* $c_{-n} = (a_n + i b_n) / 2 = \overline{c_n}$

## 13.3. Cálculo de Coeficientes

### 13.3.1. Algoritmo paso a paso
* **Paso 1:** Identificar el período $L$ (o $T$) de la función y su frecuencia angular $\omega_n = \frac{2\pi}{L} n$.
...
Se desea hallar el desarrollo en serie de Fourier de la función $f(x) = x - \lfloor x \rfloor$. Su período es $L = 1$, por lo que $\omega_n = 2\pi n$.
...
### 13.4.1. Error Puntual

$$ 
E_N(x) = f(x) - S_Nf(x)
$$

### 13.4.2. Error Cuadrático Medio

$$
E_N = \sqrt{ \frac{1}{L} \int_L |E_N(x)|^2 dx }
$$
...
* $(f(x_0^+) + f(x_0^-)) / 2$ en cada punto de discontinuidad (es decir, converge exactamente al promedio matemático de los límites laterales de ese salto).
...
* **Derivación:** Si $f$ y su derivada $f'$ son seccionalmente continuas, se puede derivar la serie término a término:
  $$
  f'(x) = \sum_{n=1}^{\infty} \bigg(- \omega_n a_n \sin(\omega_n x) + \omega_n b_n \cos(\omega_n x) \bigg)
  $$

* **Integración:** Definiendo $F(x) = \int_0^x f(t) dt$, se puede integrar la serie término a término:
  $$
  F(x) = \frac{a_0}{2}x + \sum_{n=1}^{\infty} \bigg(\frac{a_n}{\omega_n} \sin(\omega_n x) -\frac{b_n}{\omega_n} \cos(\omega_n x) \bigg)
  $$
